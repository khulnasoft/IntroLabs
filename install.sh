...
install_dir="/usr/share/introlabs"
bin_dir="/usr/bin"
...
if [[ $choice =~ ^[1-2]+$ ]]; then
    ...
    if curl -s -m 10 https://www.google.com > /dev/null || curl -s -m 10 https://www.github.com > /dev/null; then
        ...
        echo "";
        echo -e "${YELLOW}[✔] Downloading introlabs...${NC}"
        if sudo git clone https://github.com/khulnasoft/introlabs.git $install_dir; then
...
#            echo '#!/bin/bash' > introlabs.sh
            echo '#!/bin/bash' > $install_dir/introlabs.sh
            echo "source $install_dir/venv/bin/activate" >> $install_dir/introlabs.sh
            echo "python3 $install_dir/introlabs.py \$@" >> $install_dir/introlabs.sh
            chmod +x $install_dir/introlabs.sh
            sudo mv $install_dir/introlabs.sh $bin_dir/introlabs
            echo -e "${GREEN}[✔] Script created successfully [✔]"
        else
            echo -e "${RED}[✘] Failed to download IntroLabs [✘]"
            exit 1
        fi
    ...
    if [ -d $install_dir ]; then
        ...
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]     ✔✔✔ Now Just Type In Terminal (introlabs) ✔✔✔      [+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
    else
